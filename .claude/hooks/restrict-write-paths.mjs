...
// но не сузить его по путям. Ограничения вида «documenter правит только docs/» до этого держались
// исключительно на тексте промпта — то есть на согласии модели их соблюдать. Хук превращает их в
// механический барьер: даже если агент решит тронуть production-код, вызов Edit/Write будет отклонён.
//
// Использование (в заголовке агента, см. .claude/agents/documenter.md):
//   hooks:
//     PreToolUse:
//       - matcher: "Edit|Write"
//         hooks:
