...
- **Известный долг вне этой задачи**: `infra/cfe/CfeBorrowService.ts` при заимствовании объектов
  расширения пишет существующий XML напрямую через `fs.writeFileSync(..., 'utf-8')` (несколько
  мест), в обход `writeTextFilePreservingBomAndEol` — смешанный EOL исходного файла CFE-объекта
  этим путём не сохраняется.
