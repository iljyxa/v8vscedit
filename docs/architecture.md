...
- Флаг `changesForbidden=1` схлопывает UI-режим объекта вне поставки (код `2`, `None`) в `Locked` —
  отдельного индикатора «объект снят с поддержки, но конфигурация целиком запрещена к правке» нет,
  показывается только «на поддержке, запрещено».
