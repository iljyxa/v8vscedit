...
`CHANGES_FORBIDDEN_TITLE`, `supportIndicatorOf`, `supportLockedReasonOf`) — его читают и дерево
(запись суффикса), и `UniversalPanelViewProvider` (разбор суффикса в подсказку), и сервисы отказа
...
- Причина теперь различается на уровне текста (см. `hasChangesForbidden`/`-supportChangesForbidden`
  ...
  возврат до поиска XML/uuid) — отдельного индикатора
  «этот конкретный объект снят с поддержки (код `2`), но конфигурация целиком запрещена к правке» нет:
  все объекты под таким корнем получают один и тот же замок с причиной «конфигурация».
