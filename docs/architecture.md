...
значение `SupportMode` под этот случай не заводится: значения enum заморожены (см. ниже), и
сравнения `=== SupportMode.Locked`/`MODE_STRICTNESS`/суффикс `-support<n>` перечисляют ровно три
случая — четвёртое сломало бы все три. Причина навешивается поверх режима отдельным маркером:
`MetadataTreeProvider.applySupportDecoration` дописывает к `contextValue` суффикс
`-supportChangesForbidden` ПОВЕРХ `-support2` при взведённом флаге. Формат суффикса и тексты причины
...
(`SUPPORT_CHANGES_FORBIDDEN_SUFFIX`, `SUPPORT_SUFFIX_RE`, `CHANGES_FORBIDDEN_REASON`,
`CHANGES_FORBIDDEN_TITLE`, `supportIndicatorOf`) — его читают и дерево (запись суффикса), и
`UniversalPanelViewProvider` (разбор суффикса в подсказку), и сервисы отказа
(`MetadataMutationService`, `McpMutationGate`, панель свойств через `readonlyReason:
...
(`isConfigurationOnSupport` в `cli/commands/importConfiguration.ts` не даёт импортировать основную
конфигурацию при самом наличии `.bin`, независимо от `changesForbidden`, см. «Известные ограничения»
ниже).
...
режима (`MODE_STRICTNESS`); в лог пишется предупреждение, т.к. эталона для этого случая нет.
...
- Причина теперь различается на уровне текста (см. `hasChangesForbidden`/`-supportChangesForbidden`
  ...
  MCP-мутации, панель свойств) называют её отдельно — «изменения конфигурации запрещены в настройках
  поддержки», а не общее «на поддержке, запрещено». Индивидуальный код объекта из `.bin` (0/1/2) при
  этом по-прежнему не проверяется вовсе (ранний возврат до поиска XML/uuid) — отдельного индикатора
  «этот конкретный объект снят с поддержки (код `2`), но конфигурация целиком запрещена к правке» нет:
  все объекты под таким корнем получают один и тот же замок с причиной «конфигурация». Текст
  UI-удаления (`RemoveMetadataCommand`, `FormToolsCommands`) причину не различает и остаётся прежним —
  доработка вне объёма issue #22.
- Индикатор «снят с поддержки» не различает код `2` (объект был на поставке и снят с поддержки) и
  отсутствие данных поддержки вовсе — оба случая дают `SupportMode.None` и не подсвечиваются.
- `isConfigurationOnSupport` в `cli/commands/importConfiguration.ts` не использует
  `ParentConfigurationsParser`/`changesForbidden` — импорт основной конфигурации (не расширения)
  блокируется по самому факту наличия `Ext/ParentConfigurations.bin`, без разбора его содержимого.
