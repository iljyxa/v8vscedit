...
Замок поддержки (`SupportMode`, «снято/разрешено/запрещено» из `ParentConfigurations.bin`) хранится
по UUID объекта, а UUID нужно взять из XML — модуль сам по себе его не содержит.
...
- если владелец не найден — режим `SupportMode.None`, событие пишется в лог как
  `[support] XML объекта не найден: <папка>/<имя>`.
