...
Значок узла и признак редактируемости содержимого формы/макета (дерево, панель свойств,
`set_property_by_path`/`set_type`/`compile_form`/`edit_form` и другие MCP-инструменты содержимого)
считаются по захвату САМОЙ единицы, а не владельца — см.
[repository-file-sync.md](./repository-file-sync.md#readonly). Добавление/удаление/переименование формы
по-прежнему проверяется по владельцу — это изменение его состава.
